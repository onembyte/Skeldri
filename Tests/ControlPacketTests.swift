...
struct ControlPacketTests {
    @Test func codableRoundTrips() throws {
        ...
        let values: [ControlPacket] = [.hello(version: ProtocolVersion.current, channel: .control, client: "iPad"), .ping(id:id, sentAt:1), .pong(id:id, sentAt:1), .displays([display]), .display(display), .selectDisplay(id: display.id), .strokeBegin(id:id, style:.defaultPen, point:point), .strokePoints(id:id, points:[point]), .strokeEnd(id:id), .deleteStrokes(ids:[id]), .clear]
    ...
    @Test func videoEnvelopeRoundTripAndTruncation() throws {
        let header = VideoFrameHeader(streamID: UUID(), presentationTime: 42.5, isKeyframe: true)
