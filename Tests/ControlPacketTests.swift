...
struct ControlPacketTests {
    @Test func codableRoundTrips() throws {
        ...
        let values: [ControlPacket] = [.hello(version: ProtocolVersion.current, channel: .control, client: "iPad", sessionID: id), .authorizationRequired, .authorizationResult(approved: true), .ping(id:id, sentAt:1), .pong(id:id, sentAt:1), .videoAcknowledgement(streamID: id, sequence: 7, requiresKeyframe: false), .displays([display]), .display(display), .selectDisplay(id: display.id), .requestLectureSourceSelection, .lectureSourceSelected(lectureSource, generation: id), .lectureSourceUnavailable(reason: .closed, generation: id), .leaveLectureMode, .strokeBegin(id:id, style:.defaultPen, point:point), .strokePoints(id:id, points:[point]), .strokeEnd(id:id), .deleteStrokes(ids:[id]), .clear]
