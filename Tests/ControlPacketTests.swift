...
struct ControlPacketTests {
    @Test func codableRoundTrips() throws {
        ...
        let values: [ControlPacket] = [.hello(version: 1, channel: .control, client: "iPad"), .ping(id:id, sentAt:1), .pong(id:id, sentAt:1), .displays([display]), .display(display), .selectDisplay(id: display.id), .strokeBegin(id:id, style:.defaultPen, point:point), .strokePoints(id:id, points:[point]), .strokeEnd(id:id), .deleteStrokes(ids:[id]), .clear]
