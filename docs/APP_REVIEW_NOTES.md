...
The Mac streams one user-selected display to the iPad. The iPad sends normalized annotation strokes that appear in a transparent, click-through overlay. Optional Trackpad mode is clearly selected on the iPad and requires the user to grant macOS Accessibility consent. The app does not provide unattended access, keyboard input, audio capture, internet relay, accounts, analytics, advertising, or recording.
...
5. Toggle Trackpad mode; if desired, grant Accessibility on the Mac and test pointer input.
