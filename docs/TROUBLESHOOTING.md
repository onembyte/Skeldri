...
- **Connect immediately reports Connection lost after an upgrade:** Older installers could reuse an already-running Mac process after replacing its binary, leaving the peers on different protocol versions. Current installers stop only `DrawPadMac`, wait for shutdown, and launch the new build. Quit DrawPadMac manually once if upgrading from an affected build, then rerun the installer.
