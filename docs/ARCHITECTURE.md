...
Network.framework advertises/browses `_skeldri._tcp` on the LAN. There is no cloud endpoint, telemetry, audio, or keyboard input. The optional trackpad path is isolated behind an explicit iPad mode, validates and sequences every relative event, and uses macOS's permission-gated Quartz posting API. Mode exit, disconnection, navigation Back, and iPad suspension all release held buttons. Screen recording and pointer control remain governed solely by macOS privacy controls.
