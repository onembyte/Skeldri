...
Two TCP connections are opened to the same Bonjour service. Each begins with a control `hello` declaring protocol version and `control` or `video` channel. Incompatible versions are rejected. Control messages include hello, ping/pong, video acknowledgement/recovery requests, the available-display list, selected-display metadata, an iPad display-selection request, stroke begin/points/end, explicit stroke deletion, clear, and canvas snapshot.
