...
Trackpad events carry a monotonically increasing `UInt64` sequence and one of: relative move, pixel scroll, button transition, or reset. The Mac accepts them only while the peer has explicitly selected Trackpad mode, rejects duplicate/out-of-order sequences and non-finite values, clamps untrusted deltas, and releases all held buttons on reset, mode exit, or disconnect. Drawing and trackpad surfaces are mutually exclusive on iPad.
