...
final class PeerConnection: @unchecked Sendable {
    ...
    func send(_ data: Data) { connection.send(content: data, completion: .contentProcessed { error in if let error { SkeldriLogger.network.error("Send failed: \(error.localizedDescription)") } }) }
