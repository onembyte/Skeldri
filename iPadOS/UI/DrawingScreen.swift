...
struct DrawingScreen: View {
    @ObservedObject var model: iPadAppModel
    var body: some View {
        ZStack(alignment: .bottom) { Color.black.ignoresSafeArea(); VideoDisplayView().ignoresSafeArea(); DrawingCanvasRepresentable(model: model).ignoresSafeArea(); DrawingToolbar(model: model).padding(.bottom) }
